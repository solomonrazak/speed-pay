    ...
    <div>
       ...
       />
        ...
        <div className="flex gap-2 bg-gray-200 items-center rounded-md">
          <div className={`cursor-pointer flex items-center text-[10px] w-full h-full gap-1 px-2 ${activeTab === 'allservices' ? 'bg-white font-semibold border-1 border-gray-400 rounded-md' : ''}`} onClick={() => setActiveTab('allservices')}>
            <p>AllServices</p>
            <div className="rounded-full flex justify-center items-center h-4 w-4 bg-gray-300">54</div>
          </div>
          <div className={`flex text-[10px] gap-1 cursor-pointer items-center w-full h-full px-2 ${activeTab === 'forms' ? 'bg-white font-semibold border-1 border-gray-400 rounded-md' : ''}`} onClick={() => setActiveTab('forms')}>
            <p>Forms & Permits</p>
            <div className="rounded-full flex justify-center items-center h-4 w-4 bg-red-200 text-red-600">40</div>
          </div>
          <div className={`flex text-[10px] gap-1 cursor-pointer items-center w-full h-full px-2 ${activeTab === 'fees' ? 'bg-white font-semibold border-1 border-gray-400 rounded-md' : ''}`} onClick={() => setActiveTab('fees')}>
            <p>Fees & Charges</p>
            <div className="rounded-full flex justify-center items-center h-4 w-4 bg-yellow-100 text-yellow-500">10</div>
          </div>
          <div className={`flex text-[10px] gap-1 cursor-pointer items-center w-full h-full px-2 ${activeTab === 'trials' ? 'bg-white font-semibold border-1 border-gray-400 rounded-md' : ''}`} onClick={() => setActiveTab('trials')}>
            <p>Trials & Adverts</p>
